...
struct CBinTree{
	...
	void printVertical(CBinNode<T> *p, int indent);
...
template <class T, class C>
void CBinTree<T, C>::printVertical(CBinNode<T> *p, int indent){
	if(!p) return;
	queue<CBinNode<T> *> Q;
	...
	while(Q.empty() == false)
    {
        ...
        CBinNode<T> *node = Q.front();
        cout << node->m_Data << " ";
        Q.pop();

        if (node->m_nodes[0] != NULL)
            Q.push(node->m_nodes[0]);

        if (node->m_nodes[1] != NULL)
            Q.push(node->m_nodes[1]);
    }
...
int main(int argc, char const *argv[])
{
	...
	while(true){
		...
		if(option==1){
            ...
            test.printVertical(test.m_root,0);
			//break;
	}
